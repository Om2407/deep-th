...
export const createDataJabatan = async (req, res) => {
    const { id_jabatan, nama_jabatan, gaji_pokok, tj_transport, uang_makan } = req.body;

    // LF-102: backend validation - reject negative salary values
    if (Number(gaji_pokok) < 0 || Number(tj_transport) < 0 || Number(uang_makan) < 0) {
        return res.status(400).json({ msg: "Nilai gaji tidak boleh negatif" });
    }
...
export const updateDataJabatan = async (req, res) => {
    try {
        const jabatan = await DataJabatan.findOne({ where: { id: req.params.id } });
        ...
        // LF-102: backend validation on update too
        if (Number(gaji_pokok) < 0 || Number(tj_transport) < 0 || Number(uang_makan) < 0) {
            return res.status(400).json({ msg: "Nilai gaji tidak boleh negatif" });
        }

        if (req.hak_akses === "admin") {
            await DataJabatan.update({ nama_jabatan, gaji_pokok, tj_transport, uang_makan }, {
                where: { id: jabatan.id }
            });
        } else {
            return res.status(403).json({ msg: "Akses terlarang" });
