...
const FormEditDataPegawai = () => {
    ...
    const [designation, setDesignation] = useState(''); // LF-103
    ...
    const updateUser = async (e) => {
        ...
        try {
            ...
            formData.append('designation', designation); // LF-103

            const response = await axios.patch(`http://localhost:5000/data_pegawai/${id}`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
            setMsg(response.data.msg);
            Swal.fire({ icon: 'success', title: 'Berhasil', timer: 1500, text: response.data.msg });
            navigate('/data-pegawai');
        } catch (error) {
            setMsg(error.response.data.msg);
            Swal.fire({ icon: 'error', title: 'Gagal', text: error.response.data.msg });
        }
    };
    ...
    useEffect(() => {
        const getUserById = async () => {
            try {
                ...
                setDesignation(data.designation || ''); // LF-103
            } catch (error) {
                ...
                <div className='flex flex-col gap-9'>
                    <div className='rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
                        <div className='border-b border-stroke py-4 px-6.5 dark:border-strokedark'>
                            <h3 className='font-medium text-black dark:text-white'>Form Edit Data Pegawai</h3>
                        </div>
                        <form onSubmit={updateUser}>
                            ...
                            <div className='p-6.5'>
                                ...
                                <div className='flex flex-col md:flex-row w-full gap-3 text-center'>
                                    <div>
                                        <ButtonOne><span>Perbarui</span></ButtonOne>
                                    </div>
